...
const periodAccuracy = time.Millisecond
...
func Every(p time.Duration) AtSchedule {
	...
	p = p - time.Duration(p.Nanoseconds())%periodAccuracy // truncates up to seconds
...
func (ps periodicSchedule) Next(t time.Time) time.Time {
	return t.Truncate(periodAccuracy).Add(ps.period)
}

// At returns a schedule which reoccurs every period p, at time t(hh:ss).
//
// Note: At panics when period p is less than 1 Day, and error hh:ss format.
