...
// Most test jobs scheduled to run at 10 milliseconds mark.
// Test expects to fail after TimeWaitToFaile: 50 milliseconds.
const TimeWaitToFail = 50 * time.Millisecond
...
func TestNoEntries(t *testing.T) {
	...
	select {
	case <-time.After(TimeWaitToFail):
		t.FailNow()
	case <-stop(cron):
	}
}
...
func TestNoPhantomJobs(t *testing.T) {
	...
	cur := entry.Load()
	if cur > 1 {
		t.Errorf("phantom job had run %d time(s).", cur)
	}
}
...
func TestAddBeforeRun(t *testing.T) {
	...
	cron := New()
	cron.AddFunc(Every(10*time.Millisecond), "", func() { done <- struct{}{} })
	cron.Start()
	defer cron.Stop()
...
func TestJobsDontRunAfterStop(t *testing.T) {
	...
	cron.Start()
	cron.Stop()
	cron.AddFunc(Every(10*time.Millisecond), "", func() { wg.Done() })
...
func TestMultipleEntries(t *testing.T) {
	...
	cron.AddFunc(Every(5*time.Minute), uuid.NewString(), func() {})
	cron.AddFunc(Every(10*time.Millisecond), uuid.NewString(), func() { wg.Done() })
	cron.AddFunc(Every(10*time.Millisecond), uuid.NewString(), func() { wg.Done() })
	cron.AddFunc(Every(10*time.Millisecond), uuid.NewString(), func() { wg.Done() })
	cron.AddFunc(Every(4*7*24*time.Hour), uuid.NewString(), func() {})
...
func TestRunJobTwice(t *testing.T) {
	...
	cron.AddFunc(Every(1*time.Hour), uuid.NewString(), func() { wg.Done() })
	cron.AddFunc(Every(7*24*time.Hour), uuid.NewString(), func() { wg.Done() })
	cron.AddFunc(Every(10*time.Millisecond), uuid.NewString(), func() { wg.Done() })

	select {
	case <-time.After(2 * TimeWaitToFail):
		t.FailNow()
	case <-wait(wg):
	}
}
...
func (j arbitraryJob) JobID() string {
	return j.id
}
...
func TestJobImplementer(t *testing.T) {
	...
	cron := New()
	cron.Add(Every(24*time.Hour), arbitraryJob{wg, "job-1"}) // merely distraction
	cron.Add(Every(10*time.Millisecond), arbitraryJob{wg, "job-2"})
	cron.Add(Every(7*24*time.Hour), arbitraryJob{wg, "job-3"}) // merely distraction
...
func TestEntryOrdering(t *testing.T) {
	...
	cron.Add(Every(10*time.Millisecond), arbitraryJob{wg, "job-2"})
	cron.Add(Every(7*24*time.Hour), arbitraryJob{wg, "job-3"})
