...
func (a *ParamData) Decode(pkt []byte) error {
	...
	if fn, ok := argTable[a.ParamID]; ok {
		a.ParamValue = fn.decode(pkt, &idx, int(a.ParamLen))
	}
	return nil
}

func (a *ParamData) Encode() (pkt []byte, err error) {
	pkt = hex.WriteDoubleWord(pkt, a.ParamID)
	if fn, ok := argTable[a.ParamID]; ok {
		value := fn.encode(a.ParamValue)
...
var argTable = map[uint32]*paramFn{
	// 终端心跳发送间隔,单位为秒(s)
	0x0001: {
		decode: func(b []byte, idx *int, paramLen int) any { return hex.ReadDoubleWord(b, idx) },
		encode: func(a any) (pkt []byte) { return hex.WriteDoubleWord(pkt, a.(uint32)) },
	},
	// TCP消息应答超时时间,单位为秒(s)
	0x0002: {
		decode: func(b []byte, idx *int, paramLen int) any { return hex.ReadDoubleWord(b, idx) },
